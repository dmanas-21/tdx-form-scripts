(function () {
  ...
  function setupConditionalRequired(configEl) {
    ...
    triggerEl.addEventListener('change', validateDependents);
    triggerEl.addEventListener('blur',   validateDependents);
    if (window.jQuery) window.jQuery(triggerEl).on('change', validateDependents);

    // If the trigger is a radio group, every radio in the group
    // needs its own listener — changing the selection fires change
    // on whichever radio was just clicked, not on a shared element.
    ...
    if (triggerEl.type === 'radio' && triggerRadios.length > 1) {
      for (var r = 0; r < triggerRadios.length; r++) {
        triggerRadios[r].addEventListener('change', validateDependents);
      }
    }
    ...
    dependentIds.forEach(function (depId) {
      ...
      var input = grpEl.querySelector('input, select, textarea');
      if (input) {
        input.addEventListener('blur', validateDependents);
        input.addEventListener('input', function () {
          var errId = 'tdx-cond-required-' + depId;
          setError(grpEl, errId, '');
          setSubmit(true);
