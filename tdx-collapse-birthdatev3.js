...
(function () {
  ...
  // Show / clear the live "digits remaining" hint beneath the SSN field.
  function ssnSetHint(message) {
    var grp = document.getElementById(SSN_GRP_ID);
    if (!grp) return;
    var hint = document.getElementById('tdx-ssn-hint');
    if (message) {
      if (!hint) {
        hint = document.createElement('div');
        hint.id = 'tdx-ssn-hint';
  ...
  function initSsnMask() {
    ...
    // As the user types: format with dashes and update the live hint.
    input.addEventListener('input', function () {
      ...
      if (count === 0) {
        ...
        ssnSetHint(remaining + (remaining === 1 ? ' digit remaining' : ' digits remaining'));
      } else {
        ssnSetHint('All 9 digits entered.');
      }
      // Clear any error the moment they're typing again.
      ssnSetError('');
    });
    ...
    input.addEventListener('blur', function () {
      var count = ssnDigits(input.value).length;
      if (count === 0) {
        // Empty: let TDX's own required-check handle it on submit.
        ssnSetError('');
        ssnSetSubmit(true);
        return;
      }
    ...
    // Build the eye toggle button inside the field wrapper.
    ...
    // Pad the input so typed text clears the icon.
