(function () {
  ...
  /* ============================================================
     FEATURE 1 — COLLAPSE
     ------------------------------------------------------------
     Collapses every field that sits between a start marker and an
     end marker in the form's field order. No field IDs are listed
     anywhere — the section is defined purely by its position in
     the form, so adding or removing fields from that section in
     the TDX form builder requires zero changes to static content.

     Markers (placed in static content, in the same position in
     the field order as the section they should wrap):

       <div class="tdx-collapse-start">Section Label</div>
       ... TDX fields render here normally, in the form builder ...
       <div class="tdx-collapse-end"></div>

     Add class "tdx-open" on the START marker to begin that
     section expanded (default is collapsed).

     The start/end markers must be DIRECT SIBLINGS of the fields
     they wrap — same parent element, one after another in the
     DOM — which matches how TDX renders a flat field list.
     ============================================================ */
  ...
  // Collect every sibling element between the start marker and the
  // matching end marker, walking forward through the DOM. Stops if
  // it runs off the end of the parent without finding an end marker
  // (a misconfigured section), logging nothing collected rather
  // than collapsing the rest of the form by accident.
  function collectSectionFields(startEl) {
    var fields = [];
    var node = startEl.nextElementSibling;

    while (node) {
      if (node.classList && node.classList.contains('tdx-collapse-end')) {
        return { fields: fields, endEl: node };
      }
      fields.push(node);
      node = node.nextElementSibling;
    }

    return { fields: [], endEl: null };  // no matching end marker found
  }

  function setupCollapse(startEl) {
    var label = (startEl.textContent || 'Section').trim();
    var startOpen = startEl.className.indexOf('tdx-open') !== -1;

    var collected = collectSectionFields(startEl);
    var fields = collected.fields;
    var endEl  = collected.endEl;

    if (!endEl) return;   // misconfigured — no matching end marker, do nothing

    endEl.style.display = 'none';   // the end marker itself is never visible

    startEl.style.display      = 'flex';
    startEl.style.alignItems   = 'center';
    startEl.style.width        = '100%';
    startEl.style.textAlign    = 'left';
    startEl.style.padding      = '16px 20px';
    startEl.style.background   = CARD_BG;
    startEl.style.border       = '1px solid ' + BORDER;
    startEl.style.borderLeft   = '5px solid ' + GREEN;
    startEl.style.borderRadius = '8px';
    startEl.style.boxShadow    = '0 1px 3px rgba(0,0,0,0.06)';
    startEl.style.fontSize     = '17px';
    startEl.style.fontWeight   = '500';
    startEl.style.color        = GREEN;
    startEl.style.cursor       = 'pointer';
    startEl.style.marginBottom = '10px';
    startEl.style.userSelect   = 'none';
    ...
    function setState(open) {
      ...
      startEl.style.background = open ? CARD_BG_OPEN : CARD_BG;
    }

    var isOpen = startOpen;
    setState(isOpen);

    startEl.addEventListener('mouseenter', function () { startEl.style.background = CARD_BG_OPEN; });
    startEl.addEventListener('mouseleave', function () { startEl.style.background = isOpen ? CARD_BG_OPEN : CARD_BG; });
    startEl.addEventListener('click', function () { isOpen = !isOpen; setState(isOpen); });
  }
  ...
  /* ============================================================
     ...
     ============================================================ */
  function init() {
    document.querySelectorAll('.tdx-collapse-start').forEach(function (el) { setupCollapse(el); });
